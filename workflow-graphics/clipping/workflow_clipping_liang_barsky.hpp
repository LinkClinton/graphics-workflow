...
namespace workflows::clipping {
	...
	template <typename T>
	concept is_line = requires(T line)
	{
		{ line.begin.x } -> std::convertible_to<float>;
		{ line.begin.y } -> std::convertible_to<float>;
		{ line.end.x } -> std::convertible_to<float>;
		{ line.end.y } -> std::convertible_to<float>;
	};
	
	template <typename Line> requires is_line<Line>
